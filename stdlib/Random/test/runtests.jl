...
# TODO: removing big caused some of these tests to fail.
...
let mt = MersenneTwister()
    ...
    coll = Any[2, UInt128(128), Int(619), "string"]
...
#     local r, s
#     s = Int128(typemax(T)-1000) : Int128(typemax(T)) + 10000
#     # s is a 11001-length array
#     @test rand(s) isa Int128
#     @test sum(rand(s, 1000) .== rand(s, 1000)) <= 20
#     @test Int(typemax(T)-1000) <= rand(s) <= Int(typemax(T)) + 10000
#     r = rand(s, 1, 2)
#     @test size(r) == (1, 2)
...
@testset "reproducility of methods for $RNG" for RNG=(MersenneTwister,Xoshiro)
    ...
    @test randperm(mta,Int(10)) == randperm(mtb,Int128(10)) # cf. #16376
    @test randperm(0) == []
...
#     @test DSFMT.GF2X(jump1e20)  == DSFMT.calc_jump(Int128(10)^20)

#     # check validity of the implementation of copy(::GF2X)
#     let z = Int(1); @assert z !== z+0 end
...
#     let r2 = Future.randjump(r, Int(10)^20)
#         @test  r.seed !== r2.seed
#         Random.seed!(r2)
...
@testset "show" begin
    @testset "MersenneTwister" begin
        ...
        # TODO: broke removing big caused calc_jump to break.
        # Random.jump!(m, 2*Int(10)^20)
        ...
        @test string(m) == "MersenneTwister(123, (0, 1002, 0, 1))"

        @test m == MersenneTwister(123, (0, 1002, 0, 1))
        rand(m, Int64)
        @test string(m) == "MersenneTwister(123, (0, 2256, 0, 1, 1002, 1))"
        @test m == MersenneTwister(123, (0, 2256, 0, 1, 1002, 1))
